        ...
        {['All', 'Pending', 'Approved', 'Rejected'].map(status => (
          <button
            ...
            className={`px-4 py-2 font-medium transition-colors ${
              filterStatus === status
                ? 'text-[#253985] border-b-2 border-[#253985]'
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
                    ...
                    <div className="flex gap-2">
                      {r.status === 'Pending' && (
                        <>
                          <button
                            ...
                            className="px-3 py-1 rounded-xl bg-green-100 text-green-700 hover:bg-green-200 text-xs font-medium"
                          >
                            Approve
                          </button>
                          <button
                            onClick={() => setShowRejectModal(r)}
                            className="px-3 py-1 rounded-xl bg-red-100 text-red-700 hover:bg-red-200 text-xs font-medium"
                          >
                            Reject
                          </button>
                        </>
                      ...
                      {(user?.role === 'Admin' || user?.role === 'SuperAdmin') && (
                        <button
                          ...
                          className="px-3 py-1 rounded-xl bg-gray-100 text-gray-700 hover:bg-gray-200 text-xs font-medium"
                        >
                          Delete
                        </button>
                      )}
            ...
            {filteredItems.length === 0 && (
              <tr><td className="py-8 text-center text-gray-500" colSpan={isAccountant ? 7 : 6}>
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg space-y-4">
        ...
        <Field 
          ...
          placeholder="Commission, Training Fee, Placement Fee, etc." 
        />
        <Field 
          label="Amount (BDT)" 
          type="number" 
        ...
        <div className="flex justify-end gap-2 pt-2">
          <button 
            onClick={onClose} 
            className="px-4 py-2 rounded-2xl bg-gray-100 hover:bg-gray-200 font-medium"
          >
            Cancel
          </button>
          <button 
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg space-y-4">
        ...
        <Field 
          ...
          value={reason} 
          onChange={setReason} 
          placeholder="Explain why this income is being rejected"
          multiline
        />
        <div className="flex justify-end gap-2 pt-2">
          <button 
          ...
          <button 
            onClick={submit} 
            className="px-4 py-2 rounded-2xl bg-red-600 text-white hover:bg-red-700 font-medium"
          >
            Reject Entry
          </button>
        </div>
